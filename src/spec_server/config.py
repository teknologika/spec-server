...
class ConfigManager:
    ...
    def _load_from_env(self) -> Dict[str, Any]:
        ...
        env_mappings = {
            ...
            f"{self.ENV_PREFIX}AUTO_DETECT_WORKSPACE": (
                "auto_detect_workspace",
                self._parse_bool,
            ),
            f"{self.ENV_PREFIX}WORKSPACE_SPECS_DIR": "workspace_specs_dir",
            ...
            f"{self.ENV_PREFIX}STRICT_VALIDATION": (
                "strict_validation",
                self._parse_bool,
            ),
            f"{self.ENV_PREFIX}ALLOW_DANGEROUS_PATHS": (
...
def detect_workspace_root() -> Optional[Path]:
    """
    Detect workspace root directory by looking for common workspace indicators.
    ...
    # Common workspace indicators (in order of preference)
    ...
    logger.debug("No workspace root detected")
    return None


def get_effective_specs_dir(config: Optional[ServerConfig] = None) -> Path:
    """
    Get the effective specs directory based on configuration and workspace detection.

    Args:
        config: Optional configuration (will load if not provided)

    Returns:
        Path to the specs directory to use
    """
    if config is None:
        config = get_config()

    # If auto-detection is disabled, use configured specs_dir
    if not config.auto_detect_workspace:
        return Path(config.specs_dir)

    # Try to detect workspace root
    workspace_root = detect_workspace_root()
    if workspace_root:
        # Use workspace-relative specs directory
        specs_dir = workspace_root / config.workspace_specs_dir
        logger.debug(f"Using workspace specs directory: {specs_dir}")
        return specs_dir

    # Fallback to configured specs_dir
    fallback_dir = Path(config.specs_dir)
    logger.debug(f"Using fallback specs directory: {fallback_dir}")
    return fallback_dir
